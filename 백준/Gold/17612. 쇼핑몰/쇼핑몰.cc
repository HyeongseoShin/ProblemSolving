...
priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, orderCmp> orders;
...
int main()
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        // 빈 계산대가 있다면
        if((int)counters.size() < k)
        {
            int counterNum = counters.size();
            counters.push({curW, counterNum, curId});
        }
        ...
        else
        {
            ...
            // 계산 끝
            orders.push({endTime, counterNum, endId});
    ...
    for(int i = 0; i < n; i++)
    {
        auto [endTime, counterNum, id] = orders.top();
        orders.pop();
        ans += (ll)(i+1) * (ll)id;
